...
function Mintnft() {
  ...
  async function createMarket() {
    console.log("here");
    const { name, description, price,collections} = formInput;
    if (!name || !description || !price || !fileUrl || !collections ) return;
  ...
  async function createSale(url) {
    ...
    const web3Modal = new Web3Modal({
      network: `https://eth-kovan.alchemyapi.io/v2/-rsx-HZE8Py7I7mOMIRCHckg3ab-xKnU`,
    });
      ...
      <view style={{display:"flex",alignItems:"center",justifyContent:"center"}}>
        ...
        >
          ...
          <input
            className="formtxtfill docs"
            type="text"
            onChange={e => updateFormInput({ ...formInput, collections: e.target.value })}
            // id="fname"
            // name="fname"
            style={{width:"100%"}}
            // value="LET'S START WITH GIVING A UNIQUE NAME TO YOUR WORK OF ART"
          ...
          <br />
          <div style={{backgroundColor:"grey",padding:"2rem",borderRadius:".5rem"}}>
          <label for="lname">price in ETh</label>
          <br />
          <input
            className="formtxtfill docs"
            type="text"
